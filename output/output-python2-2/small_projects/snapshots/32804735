...
def doJobDoSubTasks( job, startFrame, endFrame, doSubTasks ):
	...
	if job.hasChildNodes():
		for element in job.childNodes:
			if element.nodeName == "task":
				...
				taskName = element.attributes[ "title" ].value
				executeTask = 1
				if ( startFrame != -1 ) or ( endFrame != -1 ):
					result = re.compile( "[0-9]+$" ).search( taskName )
					if result:
						if startFrame != -1:
							executeTask &= int( taskName[ result.start() + 1: ] ) >= startFrame
						if endFrame != -1:
							executeTask &= int( taskName[ result.start() + 1: ] ) <= endFrame

				if 1 == executeTask:
					print globalIndent.getIndent() + "Executing Task '" + taskName + "':"

					if 1 == doSubTasks or -1 == doSubTasks:
						for subTask in element.childNodes:
							if subTask.nodeName == "subtasks":
								...
								holdJobList = doJobDoSubTasks( subTask, -1, -1, -1 )
								#print holdJobList
					...
					for subelement in element.childNodes:
						...
						if subelement.nodeName == "commands":
							...
							jobId = taskName + "_" + "%05d" % random.randint( 0, 99999 )
							jobList += [ jobId ]
							...
							for command in subelement.childNodes:
								if command.nodeName == "command":
									...
									if -1 == doSubTasks:
										cmdarray[ 0 ] += " -p 0 -progress"
									else:
										cmdarray[ 0 ] += " -progress -ssh -hosts /home/moritzm/renderhosts.txt"
...
try:
	...
	try:
		optlist, args = getopt.getopt( sys.argv[ 1: ], "s:e:h", [ "start=", "end=", "startframe=", "endframe=", "help", "nosubs", "nosubjobs", "sge" ] )
	except:
		usage()
		sys.exit( 2 )
	...
	try:
		...
		try:
			title = scriptJob.getElementsByTagName( "jobscript" )[ 0 ].attributes[ "title" ].value
		except: # for backwards compatibility
			title = scriptJob.getElementsByTagName( "renderscript" )[ 0 ].attributes[ "title" ].value

		print "Running commands for '" + title + "':"

		doJobDoSubTasks( scriptJob.getElementsByTagName( "subtasks" )[ 0 ], startFrame, endFrame, doSubTasks )
