...
def PixmapFromPath(storage, path):
    if (storage, path) in pixmaps:
        ...
        tmp = cairo.ImageSurface.create_from_png(str(pathx))#gtk.gdk.pixbuf_new_from_file(str(pathx))
	
        pixmaps[(storage, path)] = tmp
    return tmp

# converts Hex colors(HTML format) to RGB
# should be used for setting the fg, bg color
...
class CCairoCanvas(CAbstractCanvas):
	...
	self.storage = storage
        self.pango_ctx = pangocairo.CairoContext(self.cr)
	self.pango_layout = self.pango_ctx.create_layout()
	self.fonts = {}
