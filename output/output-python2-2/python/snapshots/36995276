...
if master:
    ...
    for i in range(len(hosts)):
        ...
        for mod in range(2*i,2*i+2):
            ...
            cmd = 'ssh %s "( nice -n +19 python python/pick_homs_make_fragments.py %s %d %d %d %s > %s 2> %s.err & )"'\
...
#    NN_EXE = '/users/rhiju/nnmake_old/pNNMAKE.gnu' #phil's nnmake code, increase number of homologs allowed in exclusion list
    #NN_EXE = '/users/divyab/fragpicker/nnmake/pNNMAKE.gnu'
    ...
    for target_dir in dir_list:
        ...
        hom_fasta_files = glob('%s/%s.fasta.?.blast_h???_.fasta'\
                               %(target_dir,id))
        ...
        for file in hom_fasta_files:
            ...
            dir = target_dir+hom_id+'/'
            mkdir(dir)
            if round == 1:
                cmd = 'cp %s %s/%s.fasta'%(file,dir,hom_id)
                #print cmd
                system(cmd)
