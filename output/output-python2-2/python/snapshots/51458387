#-*- coding: utf-8 -*-
# Copyright (c) 2001-2004 Twisted Matrix Laboratories. Modified under the MIT License.
"""Support for results that aren't immediately available.

Yoinked from twisted.internet.defer, then modified.
"""
...
def timeout(deferred):
	deferred.errback(TimeoutError("Callback timed out"))
...
class Deferred(object):
	...
	def errback(self, fail=None):
		...
		*success*-callback chain.
		...
		If the argument that's passed to me is not a Exception instance,
		it will be embedded in one. If no argument is passed, a Exception
...
class DebugInfo(object):
	...
	def __del__(self):
		...
		if self.failResult is not None:
			msg = "Unhandled error in Deferred: "
			debugInfo = self._getDebugTracebacks()
			if debugInfo != '':
				msg += "(debug: " + debugInfo + ") "
			msg += repr(self.failResult)
			logging.error(msg)
	...
	def _cbDeferred(self, result, index, succeeded):
		...
		if not self.called:
			if succeeded == SUCCESS and self.fireOnOneCallback:
				...
				self.errback(FirstError(result, index))
	...
	while 1:
		try:
			...
			if isinstance(result, Exception):
				result = g.throw(type(result), result)
			else:
