#!/usr/bin/env python
"""
Given a directory containing head and .cat files as produced by sextractor,
create the exposureList files and the DIASource database tables for use
by the Association pipeline

Run as SexToLSST.py dbName dirName expfileName

NEED TO handle errors from mysql - eg timeouts!  Possibly do consistency check on # of rows
...
def createDIASourceTable(obsNum, filterNum, expTime, catFile, mySqlHost, mySqlUser, mySqlPasswd, mySqlDb):
    maxChunk = 1000
...
#    if debug: print createTableCmd
    ...
    magAdjust = 2.5*math.log10(expTime)
        ...
        for line in chunkLines:
            if commentRE.match(line)==None:
                ...
                if int(flag)==0:
                    ...
                    insertCmd += '(%d,%d,%d,%.5f,%.5f,%.3f,%.3f),' % (id, obsNum, filterNum, float(ra), float(dec), float(mag)+magAdjust, float(magErr))
                    nSources += 1
...
for headFile in headList:
    catFile = headSuffixRE.sub('.cat', headFile)
    ...
    createDIASourceTable(obsNum, filtNum, float(exp), catFile, mySqlHost, mySqlUser, mySqlPasswd, mySqlDb)
