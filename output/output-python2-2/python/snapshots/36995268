...
def Help():
    ...
    print ' Extracts cluster members of lowest energy from specified clusters (with 0 being'
    print '  the number of the first cluster. Suffix is .2.pdb.'
    print
    exit()
...
for clusternum in clusternums:
    ...
    count = 0
    for clustermember in clustermembers[1]:
...
for outfilename in outfilenames:
    ...
    for tag in taglist[outfilename]:
        fid.write(tag+'\n')
    fid.close()
    command = '/users/rhiju/rosetta++/rosetta.gcc -extract -l temp.list -paths /users/rhiju/paths.txt -s '+outfilename
