    ...
    def check_modules(self):
        ...
        if opts.mwg_ssl_dir:
            check_module("QtAssistant", "qassistantclient.h",
                    ...
                    extra_lib_dirs=[os.path.join(opts.mwg_ssl_dir, "lib")],
                    extra_libs=["ssleay32", "libeay32"])
        else:
            check_module("QtAssistant", "qassistantclient.h",
                    "new QAssistantClient(\"foo\")")
...
def check_dbus():
    ...
    _, sout, _ = os.popen3("pkg-config --cflags-only-I --libs dbus-1")
...
def check_module(mname, incfile, test, extra_include_dirs=None, extra_lib_dirs=None, extra_libs=None):
    ...
    if os.access(os.path.join("sip", mname, mname + "mod.sip"), os.F_OK):
        ...
        if check_api(incfile, test, mname, extra_include_dirs=extra_include_dirs, extra_lib_dirs=extra_lib_dirs, extra_libs=extra_libs):
            ...
            sipconfig.inform("Module '%s' not avaiable!" % mname)
...
def manage_cache(mname):
    ...
    """
    import os.path
    sipconfig.inform("Managing the %s module cache..." % mname)
    for dirname in ["cache", "cache/"+mname]:
        try:
            os.mkdir(dirname)
        except:
            pass

    def isDifferent(filename1, filename2, start=0):
        try:
                ...
                if len(lines1) != len(lines2):
                    ...
                    for i in range(len(lines1)):
                        if lines1[i] != lines2[i]:
                            if lines1[i].find("* Generated by SIP") == -1:
                                return True
                    else:
                        return False
        except OSError:
    ...
    for newfilename in glob.glob(mname+"/*.c*") + glob.glob(mname+"/*.h"):
