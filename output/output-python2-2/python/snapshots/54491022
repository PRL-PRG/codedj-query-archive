...
# $Id: ID3.py,v 1.2 2003/06/05 11:35:23 chrisshaffer Exp $
...
class ID3:
	...
	def setup_dict(self):
		...
		if self.track: self.d["TRACKNUMBER"] = self.tupleize("%02d" % (self.track)) #str(self.track))
	...
	def __str__(self):
		if self.has_tag:
			...
			if self.track != None:
				track = "%02d" % self.track #str(self.track)
			else:
				track = 'Unknown'
